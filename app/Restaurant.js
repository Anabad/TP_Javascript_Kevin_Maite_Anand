...
const TEMPS_PREPARATION_MAX = 10 * 1000; // Correspond à 50 minutes dans notre horloge
...
module.exports = class Restaurant {
    constructor(horaire) {
        this.recette = this.__creerRecette();
    ...
    __creerRecette() {
        var recette = creationTableau(getRandom(NOMBRE_DE_RECETTE_MIN, NOMBRE_DE_RECETTE_MAX),[]);
        for (var i = 0; i < recette.length; i++) {
            recette[i].push(remplirTableauAleatoire(creationTableau(NOMBRE_TYPE_INGREDIENT,0),getRandom(NOMBRE_INGREDIENT_MIN_RECETTE,NOMBRE_INGREDIENT_MAX_RECETTE)));
        }
        return recette;
    }
    ...
    __listeRepasDispo() {
        ...
        for (var i = 0; i < this.recette.length; i++) {
            if (this.stock.__resteAssezIngredientIndice(this.recette[i])) {
                repasDispo.push(i);
            }
        }
        return repasDispo;
    ...
    __notationRestaurant(client) {
        // Si le client est servi 10 minutes avant son seuil de résistance alors le restaurant gagne 2 points
        if (client.attente < client.seuilDeResistance - 10) {
            this.note = this.note + 2;
        }
        // Si le client est servi au maximum 5 minutes après son seuil de résistance le restaurant gagne 1 point
